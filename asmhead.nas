...
VBEMODE	EQU		0x4144			; 1024 x  768 x 8bit 彩色 105
; 显示模式
...
; --- 2. 【核心修改】在切换模式前，立刻把信息搬走 ---
		...
		MOV		AL,[ES:DI+0x19]
		MOV		[0x0ff3],AL			; 存位数 (BPP)
		
		MOV		AX,[ES:DI+0x10]
		MOV		[0x0ffc],AX			; 存关键的 Pitch

		MOV		EAX,[ES:DI+0x28]
		MOV		[VRAM],EAX			; 存最关键的 VRAM 地址
...
;	画面设置
		...
		MOV		AX,0x4f02
		INT		0x10
		MOV		BYTE [VMODE],2	; 屏幕的模式（参考C语言的引用）
		MOV		AX,[ES:DI+0x12]
		MOV		[SCRNX],AX
		MOV		AX,[ES:DI+0x14]
		MOV		[SCRNY],AX
