...
putfonts8_sht(sht_back,120,16,"int 2e",0);
   ...
   status = io_in8(pcbar->IDE_DMA_BASE + 2);
    io_out8(pcbar->IDE_DMA_BASE + 2, status | 0x04); // 往 Bit 2 写 1 即可清零该位
...
//unsigned int  pcbar->NET_DMA_BASE;
void init_ide_pci() {
	struct GATE_DESCRIPTOR    *idt = (struct GATE_DESCRIPTOR    *) ADR_IDT;
	unsigned int bus;
	unsigned int dev;
	unsigned int func;
	pcbar->NET_DMA_BASE=0;
	pcbar->IDE_DMA_BASE=0;
	int tt=0;
	char temp[64];
	char s[50]={0};
    ...
    if (bar0 & 0x01) {
        // IO Port 模式
        pcbar->NET_DMA_BASE = bar0 & 0xFFFFFFFC;
    } else {
        // MMIO 模式
         pcbar->NET_DMA_BASE = bar0 & 0xFFFFFFF0;
    }
...
cmd &= ~(1 << 10); // 确保 Interrupt Disable 位被清零 (开启中断)
    pci_write(bus, dev, func, 0x04, cmd);
  cmd = pci_read(bus, dev, func, 0x04);
    // 3. 获取中断号 (用于处理收包)
	if (cmd & (1 << 10)) {
    putfonts8_ascAll(200, 360, "PCI INT DISABLED! FIXING...");
    pci_write(bus, dev, func, 0x04, cmd & ~(1 << 10)); // 强制清零，开启中断
}
    unsigned int irq = pci_read(bus, dev, func, 0x3C) & 0xFF;
	unsigned int vector = 0x20 + irq;
	set_gatedesc(idt + vector, (int) asm_inthandler29, 2 * 8, AR_INTGATE32);

unsigned int low =  (*(volatile unsigned int *)(pcbar->NET_DMA_BASE + 0x5400));
unsigned int high = (*(volatile unsigned int *)(pcbar->NET_DMA_BASE + 0x5404));
...
putfonts8_ascAll(200, 220, s);
	...
	if(tt==2){return; }// 找
            ...
            if (base_class == 0x01 && sub_class == 0x01) {
               ...
               pcbar->IDE_DMA_BASE = bar4 & 0xFFFFFFFC; // 屏蔽低位标志
...
void ide_dma_read(unsigned int lba, unsigned char count, char *buf) {
    if (pcbar->IDE_DMA_BASE == 0 ) return; // 没扫到硬件
	dma_done_flag=0;
    // 1. 设置 PRD 表（注意：buf 必须是物理地址）
    setup_prd(buf, count * 512);
    io_out32(pcbar->IDE_DMA_BASE + 4, (unsigned int)&my_prd);

    // 2. 准备 DMA 控制器 (读方向 + 清理状态)
    io_out8(pcbar->IDE_DMA_BASE + 0, 0x08); 
    io_out8(pcbar->IDE_DMA_BASE + 2, 0x06);
    ...
    io_out8(pcbar->IDE_DMA_BASE + 0, 0x08 | 0x01); // START!
...
void ide_dma_write(unsigned int lba, unsigned char count, char *buf) {
    if (pcbar->IDE_DMA_BASE == 0) return;
    ...
    io_out8(pcbar->IDE_DMA_BASE + 0, 0x00); 
    io_out8(pcbar->IDE_DMA_BASE + 2, 0x06); // 清理状态位
    ...
    io_out8(pcbar->IDE_DMA_BASE + 0, 0x01); 

    // 5. 轮询等待完成
    while (1) {
        unsigned char status = io_in8(pcbar->IDE_DMA_BASE + 2);
